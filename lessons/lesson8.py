...
# list collection
# ordered collection that can be changable and allow duplicates
...
# range of the list
# range of values in a list is the same in strings an looping
...
# The remove function will remove from a specific value from the list
def simple_remove():
    colors.remove('green')
    for color in colors:
        print(color)
...
# not picking the index will take the last item
def simple_pop():
    colors.pop();
    for value in colors:
        print(value)
...
# can also delete a list entirely
def simple_del():
    del colors[3]
    for color in colors:
        print(color)
...
# clear method will empty list, but not remove the list
def simple_clear():
    mixed.clear()
    mixed.append('Python')
    for mix in mixed:
...
# change a value in a tuple, but converting to a list then back again
...
# Joining set collections by update, union and pipe | operator
even = {2,4,6,8,10}
odd = {1, 3, 5, 7, 9}
tens = {10, 20, 30, 40, 50}
...
# Dictionaries are collections consisting of a key in value
